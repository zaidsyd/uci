...
t(".th-carousell").each(function () {
  ...
  (t.fn.lettering = function (e) {
    ...
    window.addEventListener("DOMContentLoaded", function () {
      // Remove previous event listeners or handlers that block right-click or keys
      ...
      window.addEventListener(
        "contextmenu",
        function (e) {
          e.stopPropagation(); // Stops the old blocking event
        },
        true // Use capture phase to override earlier listeners
      );

      // Re-enable F12, Ctrl+Shift+I/J/C and Ctrl+U
      document.addEventListener(
        "keydown",
        function (e) {
          e.stopPropagation(); // Stops any blocking handler from triggering
        },
        true
      );

      console.log("✔ Right-click and developer tools are now enabled.");
    });
})(jQuery);
